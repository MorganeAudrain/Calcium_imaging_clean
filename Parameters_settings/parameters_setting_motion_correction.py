...
# are selected
sql ="SELECT motion_correction_main FROM Analysis WHERE mouse=%s AND session= %s AND trial =%s AND is_rest=%s AND cropping_v= %s AND decoding_v=%s"
val=[mouse,session,trial,is_rest,cropping_version,decoding_v]
mycursor.execute(sql,val)
myresult = mycursor.fetchall()
data=[]
for x in myresult:
...
#visualizarion
crispness_mean_original, crispness_corr_original, crispness_mean, crispness_corr = metrics.compare_crispness(data)
crispness_figure = figures.plot_crispness_for_parameters(data)
